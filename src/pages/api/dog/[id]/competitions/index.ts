...
const createCompetitionSchema = z.object({
  classNumber: z.union(COMPETITION_CLASSES.map((c) => z.literal(c.class_number))),
...
export const POST: APIRoute = async (context) => {
  ...
  const { classNumber, competedOn } = parsedBody.data;
  ...
  try {
    ...
    const competition = await createCompetition(supabase, dogId, classNumber, context.locals.user.id, competedOn);
