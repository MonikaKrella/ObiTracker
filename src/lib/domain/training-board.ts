 ...
 * of a dog's training elements. `create()` fails fast on a tick referencing
 * an element outside this board's set, rather than silently trusting the
 * caller to have pre-filtered the data.
 */
export class TrainingBoard {
  ...
  static create(elements: TrainingElement[], ticks: TickRecord[]): TrainingBoard {
    ...
    for (const tick of ticks) {
      const currentCount = counts.get(tick.elementId);
      if (currentCount === undefined) {
        throw new UnknownElementTickError(tick.elementId);
      }
      counts.set(tick.elementId, currentCount + 1);
    }
  ...
  highlights(): ReadonlyMap<string, HighlightColor> {
    const result = new Map<string, HighlightColor>(this.elements.map((e) => [e.id, null] as const));
    ...
    const byDesc = [...this.tickCounts.entries()].sort((a, b) => b[1] - a[1]);
    const byAsc = [...this.tickCounts.entries()].sort((a, b) => a[1] - b[1]);
    ...
    for (const count of this.tickCounts.values()) {
